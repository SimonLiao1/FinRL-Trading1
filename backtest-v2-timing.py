...
timing strategy is used to generate the probability of the portfolio.
...
import os
from utility import run_backtest  # to run the timing strategy
...
def compute_metrics(ret: pd.Series,
    ...
    equity = equity / equity.iloc[0]   # align to 1 at start_date
    equity = equity.loc[: "2024-12-31"]   # end at 2024-12-31
...
def generate_comparison_charts(daily_returns_map: Dict[str, pd.Series], out_dir: str,
                               rf_annual: float, mar_annual: float, labels: list):
    """
    Build rolling comparison charts for all KPIs across provided labels.
    Produces charts for frequencies M/Q/Y using default rolling windows.
    Only computes IR_vs_QQQ when "QQQ" is present in labels.
    """
    for freq, win in _ROLL_WINDOWS.items():
        ...
        # Prepare rolling KPIs
        base = labels[0]
        idx = ret_map[base].index
        ...
        ir_spx_df  = pd.DataFrame(index=idx) if "SPX" in labels else None
        ir_qqq_df  = pd.DataFrame(index=idx) if "QQQ" in labels else None
        ...
        for k in labels:
            ...
            def _sharpe(x):
                mu = np.mean(x)*n
                sd = np.std(x, ddof=0)*np.sqrt(n)
                rf = rf_annual
                return (mu - rf)/sd if sd>0 else np.nan
            sharpe_df[k] = roll.apply(_sharpe, raw=True)

            def _sortino(x):
                rf = rf_annual
                mar_p = (1+rf)**(1/n) - 1 if n>0 else 0.0
                downside = x[x < mar_p]
                dd = np.std(downside, ddof=0)*np.sqrt(n) if downside.size else np.nan
                mu = np.mean(x)*n
                return (mu - rf)/dd if dd and dd>0 else np.nan
            sortino_df[k] = roll.apply(_sortino, raw=True)
            ...
            if ir_spx_df is not None and k != "SPX":
            ...
            if ir_qqq_df is not None and k != "QQQ":
        ...
        if ir_spx_df is not None:
            export_list.append(("IR_vs_SPX", ir_spx_df))
        if ir_qqq_df is not None:
            export_list.append(("IR_vs_QQQ", ir_qqq_df))

        for metric_name, df in export_list:
            # Keep only the provided labels (and ensure deterministic order)
...
# ------------------------------- Generate timing probability file -------------------
def generate_timing_prob_file(best_params, input_path="./outputs/vix_z.csv", output_path="./outputs/daily_probabilities_spx_vix_z.csv"):
    """
    generate timing probability file for Portfolio backtest using

    Parameters
    ----------
    best_params : dict
        best parameters dictionary, for example:
        {
            "zscore_thr": 2.5,
            "dzscore_thr": 2.0,
            "dzscore_neg_thr": -2.0,
    ...
    df['spx_ret'] = df['spx_close'].pct_change().fillna(0)
    df['port_ret'] = df['spx_ret']

    # run backtest
    res = run_backtest(df.copy(), best_params)

    # save Prob file
    out_df = pd.DataFrame({"date": res["date"], "prob": res["position"]})
    out_df.to_csv(output_path, index=False)

    print(f"✅ Prob file saved to {output_path}")
    return out_df
...
def main():
    ...
    parser.add_argument("--qqq", choices=["YES","NO"], default="YES", help="Include QQQ series (charts/CSVs/IR_vs_QQQ). NO to exclude.")
    ...
    include_qqq = (getattr(args, "qqq", "YES").upper() == "YES")
    # If QQQ is excluded but user picked it as IR benchmark, fallback to SPX
    if not include_qqq and getattr(args, "ir_benchmark", "SPX") == "QQQ":
        logging.warning("`--ir-benchmark QQQ` ignored because --qqq=NO; fallback to SPX.")
        args.ir_benchmark = "SPX"
    ...
    generate_timing_prob_file(best_params)

    # ------------------- Load Probability -------------------
    try:
        prob_df = pd.read_csv("./outputs/daily_probabilities_spx_vix_z.csv", parse_dates=["date"]).set_index("date").sort_index()
        prob_series = prob_df["prob"]        
        logging.info("Loaded daily_probabilities_spx_vix_z.csv with %d rows", len(prob_series))
    except Exception as e:
        logging.warning("Probability file not found or invalid, skip prob-control. %s", e)
        prob_series = None

        # ------------------- Align Start Date -------------------
    start_date = pd.Timestamp("2018-03-01")

    spx_ret_daily   = spx_ret_daily.loc[spx_ret_daily.index >= start_date]
    if include_qqq:
        qqq_ret_daily = qqq_ret_daily.loc[qqq_ret_daily.index >= start_date]
    mean_ret_daily  = mean_ret_daily.loc[mean_ret_daily.index >= start_date]
    min_ret_daily   = min_ret_daily.loc[min_ret_daily.index >= start_date]
    equal_ret_daily = equal_ret_daily.loc[equal_ret_daily.index >= start_date]
    drl_ret_daily   = drl_ret_daily.loc[drl_ret_daily.index >= start_date]

    if prob_series is not None:
        prob_series = prob_series.loc[prob_series.index >= start_date]
    """
    def apply_prob_control(ret_series: pd.Series, prob: pd.Series,
                           low_th: float=0.3, high_th: float=0.6,
                           low_w: float=1.0, mid_w: float=0.7, high_w: float=0.5) -> pd.Series:
      
        Adjust portfolio returns based on probability thresholds.
        - prob >= high_th  → reduce exposure (e.g., 50%)
        - prob <= low_th   → full exposure (100%)
        - otherwise        → medium exposure (e.g., 70%)
     
        if prob is None:
            return ret_series
        adj = []
        for dt, r in ret_series.items():
            p = prob.loc[dt] if dt in prob.index else np.nan
            if np.isnan(p):
                adj.append(r)
            elif p >= high_th:
                adj.append(r * high_w)
                ...
                adj.append(r * mid_w)
        return pd.Series(adj, index=ret_series.index)
    """
    ...
    if include_qqq:
        returns_dict["QQQ"] = resample_returns(qqq_ret_daily, freq=freq)

    
    
    
    # ------------------- Apply Probability Control -------------------
    #prob_series = prob_series.loc[mean_ret_daily.index.min():]

    if prob_series is not None:
        start_date = min(mean_ret_daily.index.min(),
                            min_ret_daily.index.min(),
                            equal_ret_daily.index.min(),
                            drl_ret_daily.index.min())
        prob_series = prob_series.loc[start_date:"2024-12-31"]

        mean_ret_daily_prob  = mean_ret_daily  * prob_series
        min_ret_daily_prob   = min_ret_daily   * prob_series
        equal_ret_daily_prob = equal_ret_daily * prob_series
        drl_ret_daily_prob   = drl_ret_daily   * prob_series

        returns_dict.update({
            "MeanVar_prob": resample_returns(mean_ret_daily_prob, freq=freq),
            "MinVar_prob":  resample_returns(min_ret_daily_prob, freq=freq),
            "Equal_prob":   resample_returns(equal_ret_daily_prob, freq=freq),
            "DRL_prob":     resample_returns(drl_ret_daily_prob, freq=freq),
        })
    # Benchmark for IR
    ...
    summary_rows = []
    metrics_map = {}
    labels = ["SPX","MeanVar","MinVar","Equal","DRL"] + (["QQQ"] if include_qqq else [])
    for label in labels:
    ...
    for chart_f in chart_freqs:
        ...
        if include_qqq:
            daily_map["QQQ"] = qqq_ret_daily
        for label, daily_series in daily_map.items():
    ...
    if prob_series is not None:
        daily_map.update({
            "MeanVar_prob": mean_ret_daily_prob,
            "MinVar_prob":  min_ret_daily_prob,
            "Equal_prob":   equal_ret_daily_prob,
            "DRL_prob":     drl_ret_daily_prob,
        })
        labels += ["MeanVar_prob","MinVar_prob","Equal_prob","DRL_prob"]

    generate_comparison_charts(daily_map, args.out, rf_annual=args.rf, mar_annual=args.mar, labels=labels)
    ...
    # ------------------- Save Excel Summary -------------------
    try:
        xlsx_path = os.path.join(args.out, "risk_metrics_summary.xlsx")
        with pd.ExcelWriter(xlsx_path, engine="openpyxl") as writer:
            # save all strategies (including *_prob) metrics
            summary.to_excel(writer, sheet_name="Summary", index=False)

            # save equity curve for each strategy
            for label in metrics_map.keys():
                eq = metrics_map[label]["_equity"].to_frame("equity")
                eq.to_excel(writer, sheet_name=f"{label[:20]}_equity")

            # if probability strategy exists, write their metrics to a separate sheet
            if prob_series is not None:
                prob_rows = []
                for prob_label in ["MeanVar_prob","MinVar_prob","Equal_prob","DRL_prob"]:
                    if prob_label in returns_dict:
                        series = returns_dict[prob_label]
                        bmk = None if prob_label == ir_bmk_label else benchmark_series
                        m = compute_metrics(series, freq=freq, rf_annual=args.rf,
                                            mar_annual=args.mar, benchmark=bmk, label=prob_label)
                        eq = m["_equity"].to_frame("equity")
                        eq.to_excel(writer, sheet_name=f"{prob_label[:20]}_equity")
                        prob_rows.append({k: m[k] for k in [
                            "label","freq","cum_return","ann_return","ann_vol","sharpe","sortino","calmar",
                            "max_drawdown","max_dd_duration","ir","VaR_95","CVaR_95","VaR_99","CVaR_99"
                        ]})
                if prob_rows:
                    pd.DataFrame(prob_rows).to_excel(writer, sheet_name="Prob_Summary", index=False)

        logging.info("Excel summary written to %s", xlsx_path)
    except Exception as e:
        logging.warning("Failed to write Excel summary: %s", e)
